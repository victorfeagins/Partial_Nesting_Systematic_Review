...
long_exp <- raw_exp |>
  ...
  rename_with(
    ~ paste0("exp_", .x),
    .cols = !c(citation_id, study_id, response_id, experiment)
  ) |> #relabel columns to unique id columns
...
long_analytical <- raw_analytical |>
  pivot_longer(
    cols = starts_with("analytical"),
    names_to = c("experiment", ".value"),
    names_pattern = "(\\d+)_(.*)"
  ) |>
  rename_with(
    ~ paste0("analytical_", .x),
    .cols = !c(citation_id, study_id, response_id, experiment)
  ) |> #relabel columns to unique id columns
...
# Record Check
stopifnot(
  nrow(long_exp) == nrow(long_dep), #Checking to see if row numbers line up. If not there may be a data entry error on the form and should be investigated manually
  sum(long_dep$dep_present == "Yes") == nrow(long_analytical),
  long_dep |>
    filter(dep_present == "Yes") |>
    with(sum(str_detect(dep_types, "Partially Nested Clusters"))) ==
    nrow(long_partial)
)

# Joining Data
full_experiment_dat <- left_join(
  long_exp,
  long_dep,
...
full_experiment_dat <- full_experiment_dat |>
  mutate(
    pub_journal = case_when(
      study_id == 111568526 ~ "Health Psychology",
      .default = pub_journal
    )
  ) |> #Data entry error (Meta reviewer does not update)
  mutate(
    analytical_partial_cluster = case_when(
      study_id == 111578810 ~ "Ignored",
      .default = analytical_partial_cluster
    )
  ) |> #Data entry error (Meta reviewer does not update)
  mutate(
    pub_primary_secondary = case_when(
      study_id ==
        111570011 ~ "Based on data that have been previously published",
      .default = pub_primary_secondary
    )
  ) |> #Data entry error (Meta reviewer does not update)
  mutate(
    journal_type = ifelse(
      toupper(pub_journal) %in%
        c(
  ...
  mutate(
    partial_nest = ifelse(
      str_detect(dep_types, "Partially Nested Clusters"),
      "PN",
      "No-PN"
    )
  ) |>
  mutate(partial_nest = ifelse(is.na(dep_types), "No-PN", partial_nest)) #Experiments with dep_types missing means no dependency present
...
long_cond <- raw_cond |>
  pivot_longer(
    ...
    names_to = c("experiment", ".value"),
    names_pattern = "(\\d+)_(.*)"
  ) |> #Each row is experiment
  pivot_longer(
    cols = !citation_id:experiment,
    names_to = c('.value'),
    names_pattern = "(.*)\\.\\.\\.",
    names_prefix = "cond_"
  ) |> #each row is treatment condition
  ...
  mutate(
    cond_exp_units_count = case_when(
      study_id == 111569030 ~ "5",
...
pn_dat <- full_experiment_dat |>
  ...
  left_join(
    long_cond,
    by = join_by(citation_id, study_id, response_id, experiment)
  ) #adding treatment condition data which has number of group settings and treatment providers

pn_dat <- pn_dat |>
  group_by(response_id, experiment) |>
  mutate(
    cond_treatment_groups_exp = ifelse(
      is.na(cond_treatment_groups_exp),
      "No",
      cond_treatment_groups_exp
    )
  ) |> ####################TEMP LINE OF CODE DUE TO META REVIEWER DATA NOT UPDATING############################################
  mutate(
    exp_treatment_provider_percent = mean(cond_treatment_provider_exp == "Yes"),
    ...
    obs_treatment_group_percent = mean(cond_treatment_groups_obs == "Yes")
  ) |> #Should be NA for individually randomized experiments
  ungroup()


# Record Check
stopifnot(
  all(!is.na(pn_dat$exp_treatment_provider_percent)),
  all(!is.na(pn_dat$exp_treatment_group_percent))
)
  ...
  mutate(
    partial_class = case_when(
      exp_level_random == "Individual" &
        ...
        exp_treatment_group_percent > 0 ~ "Grouped IRD", #No grouping in at least one condition
      exp_level_random == "Individual" &
        exp_treatment_provider_percent < 1 &
        exp_treatment_provider_percent > 0 &
        exp_treatment_group_percent %in% c(0, 1) ~ "Treatment Provider IRD", #no grouping at all but have treatment providers in at least one condition
      exp_level_random == "Individual" &
        str_detect(partial_type, "Group Setting") &
        exp_treatment_group_percent == 1 ~ "MultiGroup IRD", # Different types of grouping or unobserved grouping
      exp_level_random == "Cluster" &
        exp_treatment_group_percent < 1 &
        exp_treatment_group_percent > 0 ~ "Grouped CRD", # Experimental units partially grouped in CRD
      exp_level_random == "Cluster" &
        exp_treatment_provider_percent < 1 &
        exp_treatment_provider_percent > 0 &
        exp_treatment_group_percent %in% c(0, 1) ~ "Treatment Provider CRD", #Experimental units partially with Treatment provider and no grouping
      exp_level_random == "Cluster" &
        obs_treatment_group_percent < 1 &
        obs_treatment_group_percent > 0 &
        exp_treatment_group_percent %in% c(0, 1) &
        exp_treatment_provider_percent %in% c(0, 1) ~ "Observational Group CRD", #Observational units partially grouped with no partially grouping or treatment providers in experimental untis
      exp_level_random == "Cluster" &
        obs_treatment_provider_percent < 1 &
        obs_treatment_provider_percent > 0 &
        obs_treatment_group_percent %in% c(0, 1) &
        exp_treatment_group_percent %in% c(0, 1) &
        exp_treatment_provider_percent %in%
          c(0, 1) ~ "Observational Treatment Provider CRD",
      .default = "Complex-PNRD"
    )
  ) #That that are those that are involve a mixed randomization procedure  or Different types of grouping or unobserved grouping
