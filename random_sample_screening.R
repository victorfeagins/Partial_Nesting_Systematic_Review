...
random_education <- screenable |> 
  ...
  filter(journal_simple %in% c("JOURNAL OF EDUCATIONAL RESEARCH", 
                               "JOURNAL OF EDUCATIONAL PSYCHOLOGY",
                               "JOURNAL OF RESEARCH ON EDUCATIONAL EFFECTIVENESS")) |> 
  nest_by(journal_simple) |> 
  rowwise() |> 
  mutate(sample = list(sample(data$ID))) |> 
  mutate(batch_1 = list(sample[1:20]), #20 reports per journal 
...
columns_need_to_add <- names(template) [!(names(template) %in% names(exportable))]
